...
#This programme uses a mix of f-strings rather than the .format() method. This can be easily changed!

def api_call(ingredient, meal_type="", allergy="", calories="", cuisine="", time=""): 
...
If your answer is red meat or tree nut, please type red-meat or tree-nut with the hyphen.")
    while True:
        ...
        list_of_allergies = ["alcohol", "celery", "crustacean", "dairy", "egg", "fish", "fodmap",
        ...
        if allergies == "":
            ...
            return formatted_allergies
        try_again = input("\nSorry I don't recognise your answer, would you like to see the allergen list? (yes/no): ")
        if try_again == "yes":
...
def time_constraints():
    ...
    while True:
        try:
            max_total_time = int(input('If you have no time constraints then please enter 0 to skip this question: '))
            if max_total_time == 0:
                return ""
        ...
        if yes_or_no == "yes":
            ...
            while True:
                ...
                if cuisine in list_of_cuisines:
                    ...
                    print("If you want to skip this question please press enter.")
        elif yes_or_no == "no":
            return ""
        else:
...
def collect_data_with_kcals(recipes):
    ...
    for item in recipes:
        ...
        data.append(f"Total calorie content: {round(item['recipe']['calories'], 2)} kcals")
        data.append(f"Calorie content per portion: {round((item['recipe']['calories'] / item['recipe']['yield']), 2)} kcals")
...
def collect_data_with_kcals_and_time(recipes):
    ...
    for item in recipes:
        ...
        for ingredient in (item['recipe']['ingredientLines']):
            data.append(ingredient)
        data.append(f"Link to Full Recipe: {item['recipe']['url']}\n")
        data.append(f"Total Time to Make: {item['recipe']['totalTime']} minutes")
        data.append("\n")
        #this is just the same data we printed out in the results. Used .append() method to add to a list
    return data  
...
def recipe_search():
    ...
    try:  
        recipes = api_call(ingredient, meal_type, allergy, calories, cuisine, time)
        if recipes == []:
            print("\nSorry there are no recipes containing {} that fit your other criteria".format(ingredient))
            search_again()
            ...
            for item in recipes:
                ...
                print('Total Time to Make:', item['recipe']['totalTime'], 'minutes')
